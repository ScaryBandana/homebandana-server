    ...
    pub fn load_or_default(path: impl Into<PathBuf>) -> Self {
        ...
        let data = match std::fs::read_to_string(&path) {
            Ok(content) => match serde_json::from_str(&content) {
                ...
                Err(error) => {
                    println!(
                        "[ConfigStore] Failed to parse configuration file '{}': {}",
                        path.display(),
                        error
                    );
                    ...
                    if std::fs::copy(&path, &backup_path).is_err() {
                        println!(
                            "[ConfigStore] Failed to create backup of existing configuration file '{}'",
                            path.display()
                        );
                    } else {
                        println!(
                            "[ConfigStore] Created backup of existing configuration file '{}'",
                            backup_path.display()
                        );
                    }
            ...
            Err(error) => {
                println!(
                    "[ConfigStore] Failed to read configuration file '{}': {}",
                    path.display(),
                    error
                );
